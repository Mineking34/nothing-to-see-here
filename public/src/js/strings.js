...
var translations = {
	...
	customSongs: {
		...
		description: {
			ja: [
				"TJA形式のcjdg譜面ファイルが入ったフォルダを選んで、カスタム曲リストで演奏しよう!"
			],
			en: [
				"Pick a folder with cjdg chart files in TJA format to play on a custom song list!"
			],
			cn: [
				"请选择一个含有cjdg谱面文件（TJA格式）的文件夹，以用于在自定义歌单中游玩。"
			],
			tw: [
				"請選擇包含cjdg譜面檔案 (TJA格式) 的資料夾，即可在自訂歌單中遊玩。"
			],
			ko: [
		...
		noSongs: {
			ja: "提供されたフォルダーにcjdg譜面ファイルは見つかりませんでした。",
			en: "No cjdg chart files have been found in the provided folder.",
			tw: "在你選擇的資料夾中找不到譜面檔案。",
	...
	plugins: {
		...
		noPlugins: {
			ja: null,
			en: "No .cjdg.js plugin files have been found in the provided file list.",
			ko: "주어진 파일 리스트에서 .cjdg.js 플러그인 파일들을 발견할 수 없습니다."
		}
	},
	search: {
		search: {
