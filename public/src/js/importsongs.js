class ImportSongs{
	...
	load(files){
		...
		for(var i = 0; i < files.length; i++){
			...
			}else if(!this.limited && (path.indexOf("/cjdg assets/") !== -1 || path.indexOf("cjdg assets/") === 0)){
				if(!(name in this.assetFiles)){
					this.assetFiles[name] = file
				}
			}else if(name.endsWith(".cjdg.js")){
	...
	addPlugin(fileObj){
		...
		var filePromise = file.read()
		return filePromise.then(dataRaw => {
			var name = file.name.slice(0, file.name.lastIndexOf(".cjdg.js"))
			this.plugins.push({
				name: name,
				data: dataRaw
			})
