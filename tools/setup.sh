...
sudo mkdir -p /srv/cjdg
sudo chown $USER /srv/cjdg
git clone https://github.com/bui/cjdg.git /srv/cjdg

cd /srv/cjdg
tools/get_version.sh
cp tools/hooks/* .git/hooks/
cp config.example.py config.py
sudo cp tools/nginx.conf /etc/nginx/conf.d/cjdg.conf
...
sudo mkdir -p /var/log/cjdg
sudo cp tools/supervisor.conf /etc/supervisor/conf.d/cjdg.conf
...
echo
echo "Setup complete! You should be able to access your cjdg instance at http://$IP"
echo
